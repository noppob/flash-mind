...
| **AI 機能の範囲** | `Card.etymology` / `Card.mnemonic` / `Card.explanation` の生成元、[import-screen.tsx](../components/screens/import-screen.tsx) の書き起こし | OpenAI API 想定（gpt-4.1-mini + Whisper）。プロンプトの仕様書は今回作らない |
